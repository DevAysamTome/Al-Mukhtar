...
var fcarimgedit = currentForm = new ew.Form("fcarimgedit", "edit");

// Validate form
fcarimgedit.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($carimg_edit->slide_src->Required) { ?>
...
fcarimgedit.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
...
fcarimgedit.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;
...
<?php $carimg_edit->showPageHeader(); ?>
<?php
$carimg_edit->showMessage();
?>
<form name="fcarimgedit" id="fcarimgedit" class="<?php echo $carimg_edit->FormClassName ?>" action="<?php echo CurrentPageName() ?>" method="post">
<?php if ($carimg_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $carimg_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="carimg">
<input type="hidden" name="action" id="action" value="update">
<input type="hidden" name="modal" value="<?php echo (int)$carimg_edit->IsModal ?>">
<div class="ew-edit-div"><!-- page* -->
<?php if ($carimg->num_slide->Visible) { // num_slide ?>
	<div id="r_num_slide" class="form-group row">
		<label id="elh_carimg_num_slide" for="x_num_slide" class="<?php echo $carimg_edit->LeftColumnClass ?>"><?php echo $carimg->num_slide->caption() ?><?php echo ($carimg->num_slide->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $carimg_edit->RightColumnClass ?>"><div<?php echo $carimg->num_slide->cellAttributes() ?>>
...
<?php if ($carimg->slide_src->Visible) { // slide_src ?>
	<div id="r_slide_src" class="form-group row">
		<label id="elh_carimg_slide_src" class="<?php echo $carimg_edit->LeftColumnClass ?>"><?php echo $carimg->slide_src->caption() ?><?php echo ($carimg->slide_src->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $carimg_edit->RightColumnClass ?>"><div<?php echo $carimg->slide_src->cellAttributes() ?>>
...
<?php if (Post("fa_x_slide_src") == "0") { ?>
<input type="hidden" name="fa_x_slide_src" id= "fa_x_slide_src" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_slide_src" id= "fa_x_slide_src" value="1">
<?php } ?>
...
<?php if (!$carimg_edit->IsModal) { ?>
<div class="form-group row"><!-- buttons .form-group -->
	<div class="<?php echo $carimg_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit"><?php echo $Language->phrase("SaveBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-href="<?php echo $carimg_edit->getReturnUrl() ?>"><?php echo $Language->phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<?php
$carimg_edit->showPageFooter();
if (DEBUG_ENABLED)
